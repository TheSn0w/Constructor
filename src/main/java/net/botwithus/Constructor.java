...
public class Constructor extends LoopingScript {
    ...
    public Constructor(String name, ScriptConfig scriptConfig, ScriptDefinition scriptDefinition) {
        super(name, scriptConfig, scriptDefinition);
        this.currentState = Constructor.ScriptState.INTERACTING_WITH_BANK;
    }
                ...
                switch (this.currentState) {
                    case INTERACTING_WITH_BANK:
                        if ("Protean Planks".equals(this.constructionItem)) {
                            ...
                            this.currentState = Constructor.ScriptState.DEPLOYING_WORKBENCH;
                        } else {
                    ...
                    case DEPLOYING_WORKBENCH:
                        if (this.deployPortableWorkbench()) {
                            ...
                            this.currentState = Constructor.ScriptState.CHECK_AND_LOGOUT_IF_NEEDED;
                        } else {
                    ...
                    case CHECK_AND_LOGOUT_IF_NEEDED:
                        if (this.checkLogoutConditions()) {
                            ...
                            this.currentState = Constructor.ScriptState.MAKING_FLATPACKS;
                        }
                        break;
                    case MAKING_FLATPACKS:
                        if (this.interactWithPortableWorkbench()) {
                            this.currentState = Constructor.ScriptState.INTERACTWITHCONSTRUCT;
                        }
                        break;
                    case INTERACTWITHCONSTRUCT:
                        if (this.InteractWithDialog()) {
                            this.currentState = Constructor.ScriptState.WAITING_FOR_COMPLETION;
                        }
                        break;
    ...
    private boolean deployPortableWorkbench() {
        ...
        if (!workbenchNearby && hasPortableWorkbenchInBackpack) {
            ...
            if (Interfaces.isOpen(1188)) {
                //[Original-Blocked]: DoAction(DIALOGUE, 0, -1, 77856776)
                MiniMenu.interact(ComponentAction.DIALOGUE.getType(), 0, -1, 77856776);
                Execution.delayUntil(5000, () -> SceneObjectQuery.newQuery().name("Portable workbench").results().nearest() != null);
            }
    ...
    private void customDelay() {
        ...
        switch (this.delayMode) {
            case "Active":
                delayValue = RandomGenerator.nextInt(600, 750);
                Execution.delay((long)delayValue);
                this.println("Active delay: " + delayValue + "ms");
                break;
            case "Intermediate":
                delayValue = RandomGenerator.nextInt(600, 1500);
                Execution.delay((long)delayValue);
                this.println("Intermediate delay: " + delayValue + "ms");
                break;
            case "AFK":
                delayValue = RandomGenerator.nextInt(600, 5000);
                Execution.delay((long)delayValue);
                this.println("AFK delay: " + delayValue + "ms");
                break;
            default:
                delayValue = RandomGenerator.nextInt(450, 750);
                Execution.delay((long)delayValue);
